...
const deadLinkGuideSlug = "uufinds-dead-link-recovery-backup-listing-workflow";
const comparisonGuideSlug = "uufinds-compare-multiple-listings-evidence-matrix";
...
const colorGuide = await readPage(`guides/${colorGuideSlug}`);
assert.match(colorGuide, /<title>UUFinds QC Color Check: Lighting &amp; Photo Limits \| UUFinds Sheet<\/title>/);
assert.match(colorGuide, /<h1>UUFinds QC Color Check: Lighting, White Balance and Photo Limits<\/h1>/);
assert.match(colorGuide, new RegExp(`<link rel="canonical" href="https://uufindssheet\\.com/guides/${colorGuideSlug}/"`));
assert.doesNotMatch(colorGuide, /hrefLang="(?:de-DE|pl-PL|pt-BR|en-GB)"|hreflang="(?:de-DE|pl-PL|pt-BR|en-GB)"/i, "Color guide must not claim a translated equivalent");
assert.match(colorGuide, /"datePublished":"2026-09-09"/);
assert.match(colorGuide, /"dateModified":"2026-09-09"/);
assert.match(colorGuide, /"@type":"Article"/);
assert.match(colorGuide, /"@type":"BreadcrumbList"/);
assert.match(colorGuide, /class="guide-table-wrap"/);
assert.doesNotMatch(colorGuide, /class="evidence-ledger"|class="source-note"|Primary source notes|Evidence and source ledger/, "Color guide must not expose research-process modules");
const colorGuideBodyStart = colorGuide.indexOf('class="guide-body"');
const colorGuideBodyEnd = colorGuide.indexOf("</article>", colorGuideBodyStart);
assert.ok(colorGuideBodyStart >= 0 && colorGuideBodyEnd > colorGuideBodyStart, "Color guide body must be extractable");
const colorGuideBody = colorGuide.slice(colorGuideBodyStart, colorGuideBodyEnd);
assert.doesNotMatch(colorGuideBody, /href="https?:\/\//i, "Color guide body must not add external links");
assert.match(colorGuideBody, /independent purchasing agent|independent purchasing agent you choose/i);
assert.match(colorGuideBody, /does not imply an affiliation|not represented as the warehouse, seller or affiliated agent/i);
const colorWordCountMatch = colorGuide.match(/data-visible-word-count="(\d+)"/);
assert.ok(colorWordCountMatch, "Color guide must expose its validated visible word count");
const colorVisibleWordCount = Number(colorWordCountMatch[1]);
assert.ok(colorVisibleWordCount >= 1200 && colorVisibleWordCount <= 1800, `Color guide word count must be 1,200–1,800, received ${colorVisibleWordCount}`);
assert.match(colorGuide, new RegExp(`"wordCount":${colorVisibleWordCount}`));
for (const locale of ["en-gb", "de", "pl", "pt-br"]) {
...
const deadLinkGuide = await readPage(`guides/${deadLinkGuideSlug}`);
assert.match(deadLinkGuide, /<title>UUFinds Dead Link Recovery &amp; Backup Listing Guide \| UUFinds Sheet<\/title>/);
assert.match(deadLinkGuide, /<h1>UUFinds Dead Link Recovery: Preserve the Source and Find a Backup Listing<\/h1>/);
assert.match(deadLinkGuide, new RegExp(`<link rel="canonical" href="https://uufindssheet\\.com/guides/${deadLinkGuideSlug}/"`));
assert.doesNotMatch(deadLinkGuide, /hrefLang="(?:de-DE|pl-PL|pt-BR|en-GB)"|hreflang="(?:de-DE|pl-PL|pt-BR|en-GB)"/i, "Dead-link guide must not claim a translated equivalent");
assert.match(deadLinkGuide, /"datePublished":"2026-09-13"/);
assert.match(deadLinkGuide, /"dateModified":"2026-09-13"/);
assert.match(deadLinkGuide, /"@type":"Article"/);
assert.match(deadLinkGuide, /"@type":"BreadcrumbList"/);
assert.match(deadLinkGuide, /class="guide-table-wrap"/);
assert.doesNotMatch(deadLinkGuide, /class="evidence-ledger"|class="source-note"|Primary source notes|Evidence and source ledger/, "Dead-link guide must not expose research-process modules");
const deadLinkGuideBodyStart = deadLinkGuide.indexOf('class="guide-body"');
const deadLinkGuideBodyEnd = deadLinkGuide.indexOf("</article>", deadLinkGuideBodyStart);
assert.ok(deadLinkGuideBodyStart >= 0 && deadLinkGuideBodyEnd > deadLinkGuideBodyStart, "Dead-link guide body must be extractable");
const deadLinkGuideBody = deadLinkGuide.slice(deadLinkGuideBodyStart, deadLinkGuideBodyEnd);
assert.doesNotMatch(deadLinkGuideBody, /href="https?:\/\//i, "Dead-link guide body must not add external links");
assert.match(deadLinkGuideBody, /independent purchasing agent|independent purchasing agent they choose/i);
assert.match(deadLinkGuideBody, /not represented as the seller, warehouse or an affiliated agent/i);
const deadLinkWordCountMatch = deadLinkGuide.match(/data-visible-word-count="(\d+)"/);
assert.ok(deadLinkWordCountMatch, "Dead-link guide must expose its validated visible word count");
const deadLinkVisibleWordCount = Number(deadLinkWordCountMatch[1]);
assert.ok(deadLinkVisibleWordCount >= 1200 && deadLinkVisibleWordCount <= 1800, `Dead-link guide word count must be 1,200–1,800, received ${deadLinkVisibleWordCount}`);
assert.match(deadLinkGuide, new RegExp(`"wordCount":${deadLinkVisibleWordCount}`));
for (const locale of ["en-gb", "de", "pl", "pt-br"]) {
  await assert.rejects(readPage(`${locale}/guides/${deadLinkGuideSlug}`), `English-only dead-link guide must not generate a fake ${locale} page`);
}

const comparisonGuide = await readPage(`guides/${comparisonGuideSlug}`);
assert.match(comparisonGuide, /<title>Compare UUFinds Listings Without Assuming They Are Identical \| UUFinds Sheet<\/title>/);
assert.match(comparisonGuide, /<h1>Compare Multiple UUFinds Listings Without Assuming They Are Identical<\/h1>/);
assert.match(comparisonGuide, new RegExp(`<link rel="canonical" href="https://uufindssheet\\.com/guides/${comparisonGuideSlug}/"`));
assert.doesNotMatch(comparisonGuide, /hrefLang="(?:de-DE|pl-PL|pt-BR|en-GB)"|hreflang="(?:de-DE|pl-PL|pt-BR|en-GB)"/i, "Comparison guide must not claim a translated equivalent");
assert.match(comparisonGuide, /"datePublished":"2026-09-17"/);
assert.match(comparisonGuide, /"dateModified":"2026-09-17"/);
assert.match(comparisonGuide, /"@type":"Article"/);
assert.match(comparisonGuide, /"@type":"BreadcrumbList"/);
assert.match(comparisonGuide, /class="guide-table-wrap"/);
assert.doesNotMatch(comparisonGuide, /class="evidence-ledger"|class="source-note"|Primary source notes|Evidence and source ledger/, "Comparison guide must not expose research-process modules");
const comparisonGuideBodyStart = comparisonGuide.indexOf('class="guide-body"');
const comparisonGuideBodyEnd = comparisonGuide.indexOf("</article>", comparisonGuideBodyStart);
assert.ok(comparisonGuideBodyStart >= 0 && comparisonGuideBodyEnd > comparisonGuideBodyStart, "Comparison guide body must be extractable");
const comparisonGuideBody = comparisonGuide.slice(comparisonGuideBodyStart, comparisonGuideBodyEnd);
assert.doesNotMatch(comparisonGuideBody, /href="https?:\/\//i, "Comparison guide body must not add external links");
assert.match(comparisonGuideBody, /independent purchasing agent|independent purchasing agent they select/i);
assert.match(comparisonGuideBody, /not represented as the seller, warehouse or an affiliated purchasing agent/i);
const comparisonWordCountMatch = comparisonGuide.match(/data-visible-word-count="(\d+)"/);
assert.ok(comparisonWordCountMatch, "Comparison guide must expose its validated visible word count");
const comparisonVisibleWordCount = Number(comparisonWordCountMatch[1]);
assert.ok(comparisonVisibleWordCount >= 1200 && comparisonVisibleWordCount <= 1800, `Comparison guide word count must be 1,200–1,800, received ${comparisonVisibleWordCount}`);
assert.match(comparisonGuide, new RegExp(`"wordCount":${comparisonVisibleWordCount}`));
for (const locale of ["en-gb", "de", "pl", "pt-br"]) {
  await assert.rejects(readPage(`${locale}/guides/${comparisonGuideSlug}`), `English-only comparison guide must not generate a fake ${locale} page`);
}
...
assert.match(articleIndex, new RegExp(`href="/guides/${deadLinkGuideSlug}/"`));
assert.match(articleIndex, new RegExp(`href="/guides/${comparisonGuideSlug}/"`));
for (const locale of locales) {
  ...
  assert.match(localizedArticleIndex, new RegExp(`href="/guides/${englishOnlyGuideSlug}/"`), `${locale} article index must route the English-only card to its canonical page`);
    ...
    assert.match(localizedArticleIndex, new RegExp(`href="/guides/${deadLinkGuideSlug}/"`), `${locale} article index must route the English-only dead-link card to its canonical page`);
    assert.match(localizedArticleIndex, new RegExp(`href="/guides/${comparisonGuideSlug}/"`), `${locale} article index must route the English-only comparison card to its canonical page`);
}
...
assert.match(sitemap, new RegExp(`https://uufindssheet\\.com/guides/${deadLinkGuideSlug}/`));
assert.match(sitemap, new RegExp(`https://uufindssheet\\.com/guides/${comparisonGuideSlug}/`));
...
assert.doesNotMatch(sitemap, new RegExp(`https://uufindssheet\\.com/(?:en-gb|de|pl|pt-br)/guides/${deadLinkGuideSlug}/`));
