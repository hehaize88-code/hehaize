...
embedded_assets="${script_dir}/embedded-binary-assets.mjs"
embedded_assets_generator="${script_dir}/generate-embedded-assets.mjs"
...
[[ -f "${pages_worker_entry}" && -f "${pages_headers}" && -f "${async_hooks_polyfill}" && -f "${empty_module}" && -f "${process_shim}" && -f "${embedded_assets_generator}" ]] || {
  echo "Missing Worker-compatible build files" >&2
  exit 69
}
...
node "${embedded_assets_generator}" "${pages_output}" "${asset_version}" "${embedded_assets}"
...
const homeResponse = await worker.default.fetch(
  new Request("https://pikobuy-sheet.net/de"),
  {
    ASSETS: {
      fetch() {
...
if (
  ...
  !homeBody.includes('lang="de"') ||
  !/hreflang/i.test(homeBody)
) {
  throw new Error("Localized Pages responses must retain versioned assets and language signals");
}

const sitemapResponse = await worker.default.fetch(
  new Request("https://pikobuy-sheet.net/sitemap.xml"),
  {
    ASSETS: {
      fetch() {
        return new Response("not found", { status: 404 });
...
if (
  ...
  !(await sitemapResponse.text()).includes("pikobuy-sheet.net")
) {
  throw new Error("Sitemap route must be available from the Pages Worker");
}
NODE
