  ...
  return (
    <InnerShell>
      ...
      <section className="catalog-layout">
        ...
        <div>
          <div className="catalog-topline"><span>PRODUCT ROUTES CHECKED AUGUST 8, 2026</span><span>USD VALUES ARE DISPLAY ESTIMATES</span></div>
          {filtered.length ? <div className="product-grid catalog-grid">{filtered.map((product) => <ProductCard product={product} key={product.id} />)}</div> : (
            <div className="empty-state"><b>No matching indexed find.</b><p>Try another term or open the live store search for a wider selection.</p><a href={`https://www.cnbuycha.com/search.html?channelid=2&keywords=${encodeURIComponent(query)}`}>Search the full catalog ↗</a></div>
          )}
        </div>
      </section>
    </InnerShell>
